...
function errorTextFormat (data) {
	...
	if(data.status == 403){
		return [false,$.i18n.prop('no_permission')];
	}
    return [false,data.statusText];
}

   function getFormatoptions(baseUrl,accessCodePrefix){
	   var formatoptions = {
			keys:true,
			editformbutton:hasPermission(accessCodePrefix,"update"),
			editbutton:hasPermission(accessCodePrefix,"update"),
			delbutton : hasPermission(accessCodePrefix,"delete"),
			editOptions:{
			   recreateForm: true,
			   url:baseUrl+'update',
			   closeOnEscape: true,
   ...
   function getGridDefaultHeight(){
	   if(HomeGlobal && HomeGlobal.gridDefaultHeight){
	...
	*/
	...
	function hasPermission(accessCodePrefix,op){
		if(accessCodePrefix){
			var accessCode = accessCodePrefix+":"+op;
...
(function ($) {
	...
	var setting = {
	        ...
	        getFitGridWidth:function(){
	        	return getFitGridWidth();
	        },
	        jsonReader: {
				root: "data.content", 
			...
			loadError:function (xhr,st,err){
				$.loy.statusErrorShow(xhr.status);
			},
			...
			gridComplete:function(){
				 var $jqGrid = $(this);
				 var width = setting.getFitGridWidth();
				 if(width == 0){
					 setTimeout(function() {
						 $jqGrid.jqGrid('setGridWidth', setting.getFitGridWidth());
					}, 20);
				 }else{
					 $jqGrid.jqGrid('setGridWidth', setting.getFitGridWidth());
				 }	
			...
			onInitGrid:function(){
				...
				$(window).on('resize.jqGrid', function() {
					 $jqGrid.loyGrid('setGridWidth', setting.getFitGridWidth());
				});
	...
	$.fn.loyGrid = $.fn.extend($.fn.jqGrid,{
		...
		navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView,pExcel) {
				o = $.extend({
				position : "left",
				closeOnEscape : true,
				beforeRefresh : null,
			...
			return this.each(function() {
				...
				for(i = 0; i<clone; i++) {
					...
					if (o.add && hasPermission(o.accessCodePrefix,"save")) {
						pAdd = pAdd || {};
					...
					if (o.edit && hasPermission(o.accessCodePrefix,"update")) {
						tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
						pEdit = pEdit ||{};
					...
					if (o.view && hasPermission(o.accessCodePrefix,"detail")) {
						tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
						pView = pView || {};
					...
					if (o.del && hasPermission(o.accessCodePrefix,"del")) {
						tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
						pDel = pDel || {};
					...
					if (o.excel && hasPermission(o.accessCodePrefix,"excel")) {
						tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
						pExcel = pExcel ||{};
						...
						.click(function(){
							if($.isFunction( o.excelfunc )) {
								...
								tableId = $.jgrid.jqID($t.p.id);
								exportExcel(pExcel.url,tableId,pExcel.removeCols);
							}
						}).hover(onHoverIn, onHoverOut);
						tbd = null;
					}
