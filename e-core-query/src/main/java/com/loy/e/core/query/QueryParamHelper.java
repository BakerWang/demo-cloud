    ...
    public static StringBuilder build(MapQueryParam mapQueryParam) {
        ...
        for (Field f : fields) {
            ...
            ConditionParam conditionParam = QueryParamAnnotationUtils
                    .getAnnotationByFieldName(clazz, attrName);
            if (conditionParam != null) {
                String paramName = conditionParam.name();
                Op opEnum = conditionParam.op();
                String op = opEnum.getInfo();
                if (value != null && value.toString() != "") {
                    ql.append(" and ").append(QueryParamHelper.rootAlias)
                            .append(QueryParamHelper.delimiter)
                ...
                ql.append(" and ").append(QueryParamHelper.rootAlias)
                        .append(QueryParamHelper.delimiter).append(attrName).append(" =:");
        ...
        if (orders.size() > 0) {
            ...
            for (Order o : orders) {
                String orderStr = QueryParamHelper.rootAlias + QueryParamHelper.delimiter + o.name()
                        + " " + o.direction().getInfo();
            ...
            if (orderProperityObj != null) {
                ...
                if (StringUtils.isNotEmpty(orderProperity)) {
                    ...
                    String orderStr = QueryParamHelper.rootAlias + QueryParamHelper.delimiter
                            + orderProperity + " " + direction;
                    tempList.add(orderStr);
                }
            }
